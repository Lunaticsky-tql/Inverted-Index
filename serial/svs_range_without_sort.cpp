...
void SvS_range_based(POSTING_LIST *queried_posting_list, int query_word_num, vector<unsigned int> &result_list) {
    ...
    for (int i = 1; i < query_word_num; i++) {
        ...
        int start_i = 0;
        int end_i = queried_posting_list[i].len - 1;
        while (start_0 <= end_0 && start_i <= end_i) {
            //move start pointer
            //"bigger head" list search the start element in the "smaller head" list
            if (result_list[start_0] >= queried_posting_list[i].arr[start_i])
                start_i = binary_search_higher_position(queried_posting_list[i].arr, result_list[start_0],
                                                        start_i, end_i);
            else
                start_0 = binary_search_higher_position(&result_list[0],
                                                        queried_posting_list[i].arr[start_i], start_0,
                                                        end_0);
            if (result_list[start_0] == queried_posting_list[i].arr[start_i]) {
                ...
                result_list[start_0] > queried_posting_list[i].arr[start_i] ? start_i++ : start_0++;
            }

            //lower tail search the end element in the "bigger tail" list
            if (result_list[end_0] <= queried_posting_list[i].arr[end_i])
                end_i = binary_search_lower_position(queried_posting_list[i].arr, result_list[end_0],
                                                     start_i, end_i);
            else
                end_0 = binary_search_lower_position(&result_list[0], queried_posting_list[i].arr[end_i],
                                                     start_0, end_0);
            if (result_list[end_0] == queried_posting_list[i].arr[end_i]) {
                ...
                result_list[end_0] < queried_posting_list[i].arr[end_i] ? end_i-- : end_0--;
            }
        }
