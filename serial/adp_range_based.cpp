...
void adp_range_based(POSTING_LIST *queried_posting_list, int query_word_num, vector<unsigned int> &result_list) {
    ...
    while(not_empty(start_index_container,end_index_container))
    {
        ...
        for(int i=0;i<query_word_num;i++){
            ...
            start_index_container[i]=binary_search_higher_position(queried_posting_list[sorted_index[i]].arr,biggest_head,start_index_container[i],end_index_container[i]);
            if(queried_posting_list[sorted_index[i]].arr[start_index_container[i]]!=biggest_head){
                flag=true;
            }
            else
